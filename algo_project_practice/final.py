...
def main_connection():
    ...
    if "The command completed successfully." not in stdout:
        path1 = r"net use v: \\fileserver2.pdc.jiit\14103151 uranidiot /USER:14103151"
        p = subprocess.Popen(path, stdout=subprocess.PIPE, shell=True)
        stdout,stderr=p.communicate('dir c:\\')
        print stdout
        if "The command completed successfully." not in stdout:
...
#loginfs("a","b")
#raw_input()
temp1.loginfs("14103151","uranidiot")
#raw_input()
conn=sqlite3.connect('V:\shubh.db')
print conn
curs=conn.cursor()
...
def copy_file(username,password):
    ...
    path = r"C:\Users\dell\Desktop\project\batch_files\fslogin.bat"
    p=subprocess.Popen([path,"uranidiot","14103151"],stdout=subprocess.PIPE,shell=True)
    stdout,stderr=p.communicate('dir c:\\')
    print stdout
    raw_input()
    ...
    while status==1:
        ...
        if(ans==counter):
            ...
            if(os.path.isfile(main_path1)):
                ...
                ans1=raw_input()
                if(ans1=='y'):
                    shutil.copy2(main_path1,r"C:\Users\dell\Documents\sm")
                ...
                ans2=raw_input()
                des = r"C:\\Users\\dell\\Documents\\sm" + l[ans-1]
                if(ans2=='c'):
...
def otp_check():
    otp=random.randint(0,100)
    text="OTP is: "+str(otp)
    messagebot.send_msg("9717020112",text)
    if(int(raw_input('Enter OTP: '))!=otp):
        return 0
    return 1
